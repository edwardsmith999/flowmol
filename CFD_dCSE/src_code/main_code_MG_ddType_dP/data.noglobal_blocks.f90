...
subroutine data_init()
      ...
      !---Secondary Layout (Note: 'noz2' because no halos in poisson)
        ...
        !--- for2D  ----
        !-- BOTH --  do kb=1,nproc
        !-- BOTH --    nlzb_(kb) = nzb_2(kb) - 2*noz2
        !-- BOTH --  end do
        !-- BOTH --  nlzb = nlzb_(irank)

        !-----Advance{u} Limits----
         i1_u = 0 + nox1
         i2_u = nlxb - nox1
         if (iblock_1.eq.1)      i1_u = 2
         if (iblock_1.eq.nbx_1)  i2_u = nlxb - 1
         niu = i2_u - i1_u + 1
        ...
        !-- BOTH --   
        !-- BOTH --   kTmin_2(1) = 1
        !-- BOTH --   kTmax_2(1) = kTmin_2(1) + (nlzb_(1)-1) -1
        !-- BOTH --   do kb=2,nbz_2
        !-- BOTH --           kTmin_2(kb) = kTmax_2(kb-1) + 1
        !-- BOTH --           kTmax_2(kb) = kTmin_2(kb) + (nlzb_(kb)-1) - 1
        !-- BOTH --   end do
        ...
        !-- x-direction
         i1_P = 1
         i2_P = nixp
         if (iblock_1.eq.1) i1_P = 0
         if (iblock_1.eq.npx) i2_P = nixp + 1
