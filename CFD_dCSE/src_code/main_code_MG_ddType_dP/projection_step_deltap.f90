...
! Update the velocity field, dp is actually dt*delta p
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	...
	do ii=i1_u, i2_u
		...
		! end if
                u(ka:kb,ii,jj)= (1-jflag_start-jflag_end) * (    ust(ka:kb,ii,jj) -           &
                                                                ( ( suxix(i,j)*0.5*(suxix(i,j)+suxix(i+1,j))                       &
                                                                   +suxiy(i,j)*0.5*(suxiy(i,j)+suxiy(i+1,j)) )*dp(ka:kb,ii  ,jj)   &
                                                                 -( suxix(i,j)*0.5*(suxix(i,j)+suxix(i-1,j))                       &
                                                                   +suxiy(i,j)*0.5*(suxiy(i,j)+suxiy(i-1,j)) )*dp(ka:kb,ii-1,jj) ) &
                                                                /( (vp(i,j)+vp(i-1,j))/2. ) - &
                                                                ( ( suxix(i,j)*(svetax(i,j+1)+svetax(i-1,j+1))/2.                  &
                                                                   +suxiy(i,j)*(svetay(i,j+1)+svetay(i-1,j+1))/2. )                &
                                                                  *( dp(ka:kb,ii,jj  )+dp(ka:kb,ii-1,jj  )                         &
                                                                    +dp(ka:kb,ii,jj+1)+dp(ka:kb,ii-1,jj+1) )/4.                    &
                                                                 -( suxix(i,j)*(svetax(i,j  )+svetax(i-1,j  ))/2.                  &
                                                                   +suxiy(i,j)*(svetay(i,j  )+svetay(i-1,j  ))/2. )                &
                                                                  *( dp(ka:kb,ii,jj  )+dp(ka:kb,ii-1,jj  )                         &
                                                                    +dp(ka:kb,ii,jj-1)+dp(ka:kb,ii-1,jj-1) )/4.         )          &
	...
	do ii=i1_v,i2_v
		...
		! end if
                ...
                v(ka:kb,ii,jj)=(1-iflag_start-iflag_end) * (     vst(ka:kb,ii,jj) -           &
                                                                ( ( svetax(i,j)*(svetax(i,j)+svetax(i,j+1))/2.                       &
                                                                   +svetay(i,j)*(svetay(i,j)+svetay(i,j+1))/2. )*dp(ka:kb,ii,jj  )   &
                                                                 -( svetax(i,j)*(svetax(i,j)+svetax(i,j-1))/2.                       &
                                                                   +svetay(i,j)*(svetay(i,j)+svetay(i,j-1))/2. )*dp(ka:kb,ii,jj-1) ) &
                                                                /( (vp(i,j)+vp(i,j-1))/2. ) - &
                                                                ( ( svetax(i,j)*(suxix(i+1,j)+suxix(i+1,j-1))/2.                     &
                                                                   +svetay(i,j)*(suxiy(i+1,j)+suxiy(i+1,j-1))/2. )                   &
                                                                  *( dp(ka:kb,ii,jj  )+dp(ka:kb,ii+1,jj  )                           &
                                                                 ...
                                                                 -( svetax(i,j)*(suxix(i  ,j)+suxix(i  ,j-1))/2.                     &
                                                                   +svetay(i,j)*(suxiy(i  ,j)+suxiy(i  ,j-1))/2. )                   &
                                                                  *( dp(ka:kb,ii  ,jj)+dp(ka:kb,ii  ,jj-1)                           &
                                                                    +dp(ka:kb,ii-1,jj)+dp(ka:kb,ii-1,jj-1) )/4.       )              &
                                                                /( (vp(i,j)+vp(i,j-1))/2. )                                         )
	...
	do ii=i1_w,i2_w
		i = ibmap_1(ii)
		j = jbmap_1(jj)
                w(ka:kb,ii,jj)=wst(ka:kb,ii,jj)-( swz(i,j)*swz(i,j)*dp(ka:kb    ,ii,jj) &
                                                 -swz(i,j)*swz(i,j)*dp(ka-1:kb-1,ii,jj) )/vp(i,j)
	end do
	end do
