...
module poisson_dct
	...
	double precision	::    p_top_halo(0:ngz,0:nlx)
	double precision	:: p_bottom_halo(0:ngz,0:nlx)
...
!------------------------------------------------------------------------------------------
	...
	!cccccccccccccccccccccccccccccccccccccccccccc
	! make pressure periodic in y directions
	!cccccccccccccccccccccccccccccccccccccccccccc
	if (BC_top .eq. 2 .and. BC_bottom .eq. 2) then
		...
		if (jblock.eq.1 .or. jblock.eq.npy) then
		    if (npy.eq.1) then
	        	p_top_halo(:,:)     = p(:,  :,   1)
	        	p_bottom_halo(:,:)  = p(:,:,nlyb-1)
			else
	        	if (jblock.eq.1) then
					p_top_halo(:,:) = p(:,  :,  1)
					call poisson_yperiodic(p_top_halo(0,0),p_bottom_halo(0,0),((ngzm+2)*(nlx+1)))
	        	else if (jblock.eq.npy) then
					p_bottom_halo(:,:) = p(:,:,nlyb-1)
					call poisson_yperiodic(p_bottom_halo(0,0),p_top_halo(0,0),((ngzm+2)*(nlx+1)))
	        	end if
			end if
		end if
