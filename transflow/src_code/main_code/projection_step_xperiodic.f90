	...
	if (BC_top .eq. 2 .and. BC_bottom .eq. 2) then
		...
		if (jblock.eq.1 .or. jblock.eq.npy) then
			if (npy.eq.1) then
				...
				ubottombc_temp(:,:,1) = u(:,:,1)
				ubottombc_temp(:,:,2) = u(:,:,2)
				vbottombc_temp(:,:,1) = v(:,:,1)
				wbottombc_temp(:,:,1) = w(:,:,1)	
		
				utopbc_temp(:,:,1) = u(:,:,nlyb-1)	
				utopbc_temp(:,:,2) = u(:, :, nlby)
				vtopbc_temp(:,:,1) = v(:,:,nlyb-1)
				wtopbc_temp(:,:,1) = w(:,:,nlyb-1)
		...
		if (jblock.eq.npy) then
			...
			v(:, :, nlby) = vbottombc_temp(:,:,1)
			w(:, :, nlby) = wbottombc_temp(:,:,1)	
		
			!Extend
			u(:,:,nlxb+1) = ubottombc_temp(:,:,2)
	...
	if (BC_top .eq. 2 .and. BC_bottom .eq. 2) then
		...
		if (jblock.eq.1 .or. jblock.eq.npy) then
			if (npy.eq.1) then
				...
				ubottombc_temp(:,:,1) = u(:,:,1)
				vbottombc_temp(:,:,1) = v(:,:,1)
				vbottombc_temp(:,:,2) = v(:,:,2)
				wbottombc_temp(:,:,1) = w(:,:,1)	
		
				utopbc_temp(:,:,1) = u(:,:,nlyb-1)	
				vtopbc_temp(:,:,1) = v(:,:,nlyb)
				vtopbc_temp(:,:,2) = v(:,:,nlyb-1)
				wtopbc_temp(:,:,1) = w(:,:,nlyb-1)
		...
		if (jblock.eq.npy) then
			...
			u(:, :, nlyb) = ubottombc_temp(:,:,1)
			w(:, :, nlyb) = wbottombc_temp(:,:,1)	
		
			!Extend
			v(:,:,nlyb+1) = vbottombc_temp(:,:,2)
