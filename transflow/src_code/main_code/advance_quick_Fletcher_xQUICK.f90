...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	...
	! TJ: arrays needed to enforce periodicty on v
	!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	real	:: v_ngxp1(1:ngzm,0:nly+1) 
	real	::  v_temp(1:ngzm,0:nly+1) 
...
!     if(ntime-ntime_.eq.1) then
	...
	! TJ: We need to enforce periodicity on v, because there is an out-of-bounds
	!     access during the computation of the explicit diffusion terms. See Eq.
	!     3.35.
	!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	!if (npx.eq.1) then
	!	
	!	v_ngxp1(1:ngzm,0:nly+1) = v(1:ngzm,2,0:nly+1) 
	!	 v_temp(1:ngzm,0:nly+1) = 0.			!Not used to enforce periodicty
	!
	...
	!
	!	v_temp(1:ngzm,0:nly+1) = v(1:ngzm,2,0:nly+1)	!this can be an arbitrary plane
	!
	!end if

	!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	! TJ: exchange v-planes
	!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	!call advancex_xperiodic(v_temp(1,0),v_ngxp1(1,0),ngzm*(nly+2))
	
	!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	! TJ: enforce periodicity on v	
	!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	!if (iblock.eq.npx) then
	!	v(1:ngzm,nlxb+1,0:nly+1) = v_ngxp1(1:ngzm,0:nly+1)
	!end if
	...
	do 61 kk=1,ngz-1
		...
		tta=(v(k,ii,jj)+v(k,ii+1,jj)+v(k,ii,jj+1)+v(k,ii+1,jj+1))/4.		!TJ-- BUG? ASK TZ. i2_u+1 is out of advance bounds?
		c1=(a1*b1)*tta 
		c2=(a1*b2)*tta 
		c4=(a2*b1)*tta
		c5=(a2*b2)*tta
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        rhs(k,ii,jj)=dt/a1*(r1*con-r2*cony(k,ii,jj)+vis)
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        cony(k,ii,jj)=con
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        rhs(k,ii,jj)=dt/vp(i,j)*(r1*con-r2*conz(k,ii,jj)+vis)
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        conz(k,ii,jj)=con
