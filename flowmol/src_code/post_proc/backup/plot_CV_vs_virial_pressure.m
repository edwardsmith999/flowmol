...
%     end
    ...
    for i = 1000:Ns:size(virial,1)-Ns
        %virial_std(i)=std(macroscopic_properties(2+(i-1)*Ns:2+i*Ns,9));
        virial_std=virial_std + (mean(virial(i:i+Ns))^2 - mean(virial)^2);
%         config_virial_std(i)  = std(virial_config(i:i+Ns));
%         kinetic_virial_std(i) = std(virial_kinetic(i:i+Ns));
    end
