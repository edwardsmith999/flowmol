...
for m =1:1:Nvflux_records-3
    ...
    [velocity_snapshot(:,:,:,:), ...
     ...
     [velocity_snapshot_tplus2(:,:,:,:)] = read_vflux(m+2,resultfile_dir,globalnbins,nd);
...
for m =1:1:Nvflux_records-2
    ...
    [energy_snapshot,energy_flux,energy_surfaces]= read_eflux(m,resultfile_dir,globalnbins);
       
    [energy_snapshot_tplus1(:,:,:,:)] = read_eflux(m,resultfile_dir,globalnbins);
    [energy_snapshot_tplus2(:,:,:,:)] = read_eflux(m+1,resultfile_dir,globalnbins);
...
%main plot
xaxis = [0:0.005:0.005*(size(dmassdt(ibin,jbin,kbin,:),4)-1)]; 
plot(linspace(0,0.695,140),a(258:end,2,1),'-','Color',[.6 .6 .6],'lineWidth', 4);
hold on
plot(linspace(0,0.695,140),a(258:end,3,1),'k--', 'markersize', 5,'lineWidth', 4);
plot(linspace(0,0.68,140/4),a(258:4:end,1,1), 'kx','lineWidth', 2,'MarkerSize',10);
Residual = squeeze(a(:,1,:))-squeeze(a(:,2,:))-squeeze(a(:,3,:));
%plot(linspace(0,0.695,140),squeeze(Residual(258:end,1)),'k-','lineWidth', 4);
set(gca,'FontSize',16)
xlabel('Time');
ylabel('Momentum')
%set(gca,'xtick',[])
axis([0.0 0.65 -0.31 0.31])
...
hold on
plot(linspace(0,0.695,140),a(258:end,3,1),'k--', 'markersize', 5,'lineWidth', 4);
plot(linspace(0,0.68,140/4),a(258:4:end,1,1), 'kx','lineWidth', 2,'MarkerSize',2);
Residual = squeeze(a(:,1,:))-squeeze(a(:,2,:))-squeeze(a(:,3,:));
%plot(linspace(0,0.695,140),squeeze(Residual(258:end,1)),'k-','lineWidth', 4);
 set(hax,'XTick',[]); 
 axis([0.0 0.65 -1.3 1.3])

%======Energy=======
scrsz = get(0,'ScreenSize');
fig3 = figure('Position',[1 scrsz(4)/4 scrsz(3)/6 scrsz(4)/2]);
plot(linspace(0,0.695,140),b(259:end,3)','-','Color',[.6 .6 .6],'lineWidth', 4)
hold on
%plot(xaxis(2:size(b,1)+1),0.5*(squeeze(b(:,1))+circshift(squeeze(b(:,1)),-1)), 'b-','lineWidth', 7)
plot(linspace(0,0.695,140),b(259:end,2)','k--', 'markersize', 5,'lineWidth', 4)
plot(linspace(0,0.68,140/4)+0.5*delta_t,b(259:4:end,1), 'kx','lineWidth', 2,'MarkerSize',10)
Residual = squeeze(b(:,3)) - 0.5*(squeeze(b(:,1))+circshift(squeeze(b(:,1)),1)) + squeeze(b(:,2));
%plot(linspace(0,0.695,140),squeeze(Residual(259:end)),'-k','lineWidth', 4);
xaxis = [0:0.005:0.005*(size(dmassdt(ibin,jbin,kbin,:),4)-1)]; 
set(gca,'FontSize',16)
...
%plot(xaxis(2:size(b,1)+1),0.5*(squeeze(b(:,1))+circshift(squeeze(b(:,1)),-1)), 'b-','lineWidth', 7)
plot(linspace(0,0.695,140),b(259:end,2)','k--', 'markersize', 5,'lineWidth', 4)
plot(linspace(0,0.68,140/4)+0.5*delta_t,b(259:4:end,1), 'kx','lineWidth', 2,'MarkerSize',2)
Residual = squeeze(b(:,3)) - 0.5*(squeeze(b(:,1))+circshift(squeeze(b(:,1)),1)) + squeeze(b(:,2));
plot(linspace(0,0.695,140),squeeze(Residual(259:end)),'-k','lineWidth', 4);
axis([0.0 0.65 -2.5 2.5])
 set(hax,'XTick',[]); 
