...
class WatchOutput:
	...
	def __init__(self,filename,figureinfo,figobj,axis):	
...
def SetupFigures(args):
	...
	try:
		...
		message = (
		            ...
		            "                     simtime Pressure \n"				 +
		  	    "Possible arguments from results/macroscopic_properties: "+
			    "iter  simtime  VSum  V^2Sum  Temp  KE  PE  TE  Pressure" 
		          )
		print(message)
		quit()
...
figureobject = plt.figure()
ax = figureobject.add_subplot(111)

# Create Figures
for f in figures:
	fig = WatchOutput(f.filename,f.figdef,figureobject,ax)

#Create legend
lgnd = []
for f in figures:
	if (len(f.figdef[1]) == 1):
		lgnd.append(f.filename+" "+f.figdef[1][0])
	else:
		for i in range(len(f.figdef[1])):
			lgnd.append(f.filename+" "+f.figdef[1][i])

ax.legend(lgnd,loc=0)
