...
program vmd_reformat
	...
	character(256) 			:: folder,filestring,filename

	!Set Constants
	call get_command_argument(1,filestring)
	if (filestring .eq. '') then
		print*, 'Input should be in the form ./vmd_reformat.exe [filepath] [filename] [no. of particles]'
	...
	write (filename,'(2a)') trim(folder), trim(filename)
	datasize = 4
