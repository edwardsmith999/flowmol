...
!-----------------------------------------------------------------------
      ...
      OPEN(42, file='results/OUTFE.DAT', status='UNKNOWN', action='WRITE',&
               iostat=IERROR(1))
      IERROR(2) = 0
      !OPEN(43, file='CPROFILE.DAT', status='UNKNOWN', action='WRITE',&
      !         iostat=IERROR(2))
      ...
      OPEN(45, file='results/ERROR.DAT', status='UNKNOWN', action='WRITE',&
               iostat=IERROR(4))
      !OPEN(46, file='VPROFILE.DAT', status='UNKNOWN', action='WRITE',&
      !         iostat=IERROR(5))
      IERROR(5) = 0
...
!       A Factorization
        CALL DGBTRF&
         (NODTOL, NODTOL, HBAND, HBAND, A, IBAND+HBAND, IPVT, INFO)

!       Aip <-- A^(-1).Aip
        CALL DGBTRS&
         ('T', NODTOL, HBAND, HBAND, 1, A, IBAND+HBAND, IPVT, A_ip, &
          NODTOL, INFO )
        ENDIF

!      B <-- A^(-1).B
       CALL DGBTRS&
        ('T', NODTOL, HBAND, HBAND, 1, A, IBAND+HBAND, IPVT, B, &
         NODTOL, INFO )
...
!     CPROFILE.DAT
       ...
       WRITE(filename,'(a16,a4)') filename,'.DAT'
...
!     TIMEDATA1.DAT
      ...
      WRITE(44,'(1X,14ES14.6)') TIME, DLOG10(TIME), XXc, DLOG10(XXc), &
                         DLOG10(XXc-1.D0), (XXc-XXco)/Dt, Hmax, Theta, Thetaa
...
!     TIMEDATA2.DAT
      ...
      WRITE(47,'(1X,14ES14.6)') TIME, DLOG10(TIME), Cao, Co, Mo, Cso, Cac, Cc, & 
                                Mc, Csc
...
50    FORMAT(I3,'. ITERATION. FULL NEWTON RAPSHON, NORM= ', E12.6)
51    FORMAT(5x,' FLAG= ', A4, '. MAX RESIDUAL= ', E12.6)
52    FORMAT(I3,'. ITERATION. MODIFIED NEWTON RAPSHON, NORM= ', E12.6)
