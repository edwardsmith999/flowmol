...
!=======================================================================
! TJ: Number of spatial phases
	real nphs_x, nphs_z

!=======================================================================
! TJ: phase-sampling flag
	integer iphs
...
! Grid-Metrics
	real suxix  (0:ngx+1,0:ngy  ),suxiy  (0:ngx+1,0:ngy  )   &
		...
		,svretax(0:ngx+1  ,0:ngy+1),svretay(0:ngx+1  ,0:ngy+1)   &
...
!=======================================================================
! TJ: arrays passed to enforce periodicity in x

      real          uleftbc_temp( 0:ngz,  0:nly, 3), urightbc_temp( 0:ngz,  0:nly, 3), &
                    vleftbc_temp( 0:ngz, 0:nly+1,3), vrightbc_temp( 0:ngz, 0:nly+1,3), &
                    wleftbc_temp(0:ngz+1, 0:nly, 3), wrightbc_temp(0:ngz+1, 0:nly, 3)

!=======================================================================
! arrays passed to enforce periodicity in y

      real          ubottombc_temp( 0:ngz,  0:nlx+1,3), utopbc_temp( 0:ngz, 0 :nlx+1,3), &
                    vbottombc_temp( 0:ngz,  0: nlx ,3), vtopbc_temp( 0:ngz, 0 : nlx ,3), &
                    wbottombc_temp( 0:ngz+1,0: nlx ,3), wtopbc_temp( 0:ngz+1,0: nlx ,3)
...
!=======================================================================
! TJ: surface texture that determines starting spanwise phase
        integer txtr_zph(0:ngx+1)

!=======================================================================
! TJ: mean flow required to compute energy
       real    U_mean  ( 0:ngz,  0:ngx+1,  0:ngy )
       real    Uy_mean ( 0:ngy )
       real    Uyz_mean( 0:ngz,0:ngy )

!=======================================================================
! TJ: Effective slip-lengths
...
!-----------------------------------------------------------
      ...
      real ::  conx(ngz-1+1, nlx-1+1, nly-1+1), &
               cony(ngz-1+1, nlx-1+1, nly-1+1), &
               conz(ngz-1+1, nlx-1+1, nly-1+1), &
	...
	!===============================================================
	parameter (memlenap_zap =ngxm*ngym*15)
	parameter (memlenap_zsuv=ngxm*ngym*12)
	parameter (memlenap_z   =ngxm*ngym*10)
	parameter (nglevel_max=6)
	! integer memlenap_zap,memlenap_zsuv,memlenap_z,nglevel_max
	!---------------------------------------------------------------
	!     I think 'vkz' should be 'vkz(2*(ngzm/2))'
	!     Note:  ngzm=niz-1
	!---------------------------------------------------------------
	real   zap_mt(memlenap_zap),vkz(ngz-1),vkx(ngx-1),omegak(ngz-1)

	real            resmin, resminb
	integer         nitmax, nitmaxb
	integer         ncycle,npre,npost,ngrid,i_allocate
	integer         iapw_mt (nglevel_max),iape_mt (nglevel_max),   &
			iaps_mt (nglevel_max),iapn_mt (nglevel_max),   &
			iapp_mt (nglevel_max),                         &
			iapws_mt(nglevel_max),iapwn_mt(nglevel_max),   &
			iapes_mt(nglevel_max),iapen_mt(nglevel_max),   &
			ivp_mt  (nglevel_max),iedge_mt(nglevel_max),   &
