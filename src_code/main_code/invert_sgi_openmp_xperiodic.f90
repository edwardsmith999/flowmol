      ...
      dimension    aa(i1_u:i2_u, j1_u:j2_u),  &
		  ...
		  rrx(i1_u:i2_u, j1_u:j2_u),  &
            rr(ngz-1, i1_u:i2_u, j1_u:j2_u)

      dimension     aa_temp(j1_u:j2_u, i1_u:i2_u), &
                    bb_temp(j1_u:j2_u, i1_u:i2_u), &
                    cc_temp(j1_u:j2_u, i1_u:i2_u), &
                   rrx_temp(j1_u:j2_u, i1_u:i2_u)

      !ccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      !TJ: I need to make aa, bb, cc, rrx, rr allocatable to 
      !    account for the different tri-diagonal solvers
      !allocatable :: aa, bb, cc, rrx
      !dimension :: rr(ngz-1, i1_u:i2_u, j1_u:j2_u)
      !ccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
! TJ: initialize coeffcient and rr, rrx arrays
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      aa  = 0.0
      bb  = 0.0
      cc  = 0.0
      ...
      do 5000 kk = 1,ngzm
      	...
      	do ii=i1_u,i2_u
        do jj=j1_u,j2_u
                rrx(ii,jj) = rhs(kk, ii, jj) !updating rrx for each kk
        end do
        end do  
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
! Neumann b.c. (only used for non-periodic flows)
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
if (BC_left.ne.2) then
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
! TJ: Solve periodic system
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

if (BC_left.eq.2) then
	...
	!TJ: tranpose aa, bb, cc, rrx in _temp arrays
	do ii=i1_u,i2_u
	do jj=j1_u,j2_u
		aa_temp(jj,ii)  =  aa(ii,jj)
		bb_temp(jj,ii)  =  bb(ii,jj)
		cc_temp(jj,ii)  =  cc(ii,jj)
		rrx_temp(jj,ii) = rrx(ii,jj) 
	end do
	end do
	
	!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	!call triPeriodicM_(id_x, aa(i1_u,j1_u),  bb(i1_u,j1_u), &
        !                         cc(i1_u,j1_u), rrx(i1_u,j1_u), niu, nju)
	!call triPeriodicM_(id_x, aa(j1_u,i1_u),  bb(j1_u,i1_u), &
        !                         cc(j1_u,i1_u), rrx(j1_u,i1_u), niu, nju)
	!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	
	call triPeriodicM_(id_x, aa_temp(j1_u,i1_u),  bb_temp(j1_u,i1_u), &
                                 cc_temp(j1_u,i1_u), rrx_temp(j1_u,i1_u), niu, nju)

	!TJ: transpose rrx_temp into rrx
	do ii=i1_u,i2_u
	do jj=j1_u,j2_u
		rrx(ii,jj) = rrx_temp(jj,ii)
	end do
	end do
	...
	do ii=i1_u,i2_u
	do jj=j1_u,j2_u
        	rr(kk,ii,jj) = rrx(ii,jj) !copying rrx into rr for each kk
	end do
	end do
...
!$OMP end parallel
      ...
      dimension ::      rrx(i1_v:i2_v, j1_v:j2_v)
      dimension ::rr(ngz-1, i1_v:i2_v, j1_v:j2_v)

      dimension ::	  aa_temp(j1_v:j2_v,i1_v:i2_v)
      dimension ::	  bb_temp(j1_v:j2_v,i1_v:i2_v)
      dimension ::	  cc_temp(j1_v:j2_v,i1_v:i2_v)
      dimension ::	 rrx_temp(j1_v:j2_v,i1_v:i2_v)
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	...
	do jj=j1_v,j2_v
       ...
       !call triPeriodicM_(id_x, aa(i1_v,j1_v),  bb(i1_v,j1_v), &
       !                         cc(i1_v,j1_v), rrx(i1_v,j1_v), niv, njv)
       !call triPeriodicM_(id_x, aa(j1_v,i1_v),  bb(j1_v,i1_v), &
       !                         cc(j1_v,i1_v), rrx(j1_v,i1_v), niv, njv)
       !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      
        call triPeriodicM_(id_x, aa_temp(j1_v,i1_v),  bb_temp(j1_v,i1_v), &
                                 cc_temp(j1_v,i1_v), rrx_temp(j1_v,i1_v), niv, njv)
	...
	do jj=j1_v,j2_v
      ...
      do ii=i1_v,i2_v
      do jj=j1_v,j2_v
        rr(kk,ii,jj) = rrx(ii,jj) !copy rrx into rr
      end do
      end do
...
!$OMP end parallel
      ...
      dimension     aa(i1_w:i2_w,j1_w:j2_w),  &
                   ...
                   rrx(i1_w:i2_w,j1_w:j2_w),  &
            rr(1:ngz-1,i1_w:i2_w,j1_w:j2_w)

      dimension     aa_temp(j1_w:j2_w,i1_w:i2_w), &
		    bb_temp(j1_w:j2_w,i1_w:i2_w), &
		    cc_temp(j1_w:j2_w,i1_w:i2_w), &
	           rrx_temp(j1_w:j2_w,i1_w:i2_w)
...
!$OMP do
!T    do 1000 kk=1,ngz-1i

!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
! TJ: initialize aa, bb, cc
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      aa = 0.0
      bb = 0.0
      cc = 0.0
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	...
	do jj=j1_w,j2_w
       ...
       !call triPeriodicM_(id_x, aa(i1_w,j1_w),  bb(i1_w,j1_w), &
       !                         cc(i1_w,j1_w), rrx(i1_w,j1_w), niw, njw)
       !call triPeriodicM_(id_x, aa(j1_w,i1_w),  bb(j1_w,i1_w), &
       !                         cc(j1_w,i1_w), rrx(j1_w,i1_w), niw, njw)
       !cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        call triPeriodicM_(id_x, aa_temp(j1_w,i1_w),  bb_temp(j1_w,i1_w), &
                                 cc_temp(j1_w,i1_w), rrx_temp(j1_w,i1_w), niw, njw)
