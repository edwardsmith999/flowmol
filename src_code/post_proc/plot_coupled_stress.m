...
%       sum(sum(sum((squeeze(totalpressure(:,:,:,ixyz)) ...
%                    -squeeze(totalflux(:,:,:,ixyz))     ...
%                    -squeeze(dvelocitydt(:,:,:,ixyz))),1),2),3)
%       
...
for m = 1:10:Nvflux_records-1
    ...
    ave_pressure_surface = zeros(nbins(1),floor(nbins(2)),nbins(3),nd);
    for j=1:t_ave
        [velocity_snapshot(:,:,:,:), ...
           velocity_flux(:,:,:,:,:), ... 
         pressure_surface(:,:,:,:,:)     ] = read_vflux(m+(j-1),resultfile_dir,gnbins,nd);
    ...
    ave_pressure_surface = ave_pressure_surface/(t_ave);
    
    %plot(0.025:0.05:0.575,squeeze(mean(mean(pressure_surface(:,2:end,:,ixyz,jxyz),1),3)),'.')   
    
    plot(xaxis(1:(gnbins(jxyz))), -squeeze(mean(mean(ave_pressure_surface(:,:,:,ixyz),1),3)), ...
         'x','LineWidth',5,'Color',[.5 .5 .5],'MarkerSize',20);
    hold on    
       
    %axis([-0.1 1.1 -0.5 0.1])
    %Plot continuum stress profile
    %plot(xaxis(gnbins(jxyz)-overlap:end),continuum_stress(:,m),'s','Color',[.5 .5 .5],'MarkerSize',20,'LineWidth',5);
...
% 
% timestep = 5;
% [velocity_snapshot(:,:,:,:,timestep)] = read_vflux(timestep,resultfile_dir,gnbins,nd);
% 
%   
% %Verify adjacent surface values are equal
% %Random Bin
% for i=1:nd
%     randbin(i) = round(rand*(gnbins(i)-1))+1;
% end
% scatter(0:gnbins(1)-1,velocity_flux(:,randbin(2),randbin(3),1,1)+...
%                         pressure_surface(:,randbin(2),randbin(3),1,1),'rx')
% hold on
% scatter(1:gnbins(1),-velocity_flux(:,randbin(2),randbin(3),1,4)+...
...
% %     plot(xaxis(1:(gnbins(ixyz)-1)/2),ave_vel_slice(:,1,m)/u_wall,'x','LineWidth',5,'Color',[.5 .5 .5],'MarkerSize',20);
...
% %     plot(xaxis(((gnbins(ixyz)-1)/2)+1-overlap:end-1),continuum_velslice(2:ny+1,m)/u_wall,'s','Color',[.5 .5 .5],'MarkerSize',20,'LineWidth',5);
% %     %Plot Halo values
% %     plot(xaxis(end),continuum_velslice(end,m+1)/u_wall,'o','Color',[.5 .5 .5],'MarkerSize',20,'LineWidth',3,'HandleVisibility','off');
% %     plot(xaxis(((gnbins(ixyz)-1)/2)-overlap),continuum_velslice(1,m+1)/u_wall,'o','Color',[.5 .5 .5],'MarkerSize',20,'LineWidth',3);
% % 
