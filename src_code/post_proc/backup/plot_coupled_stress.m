...
for m = 1:10:Nvflux_records-1
    ...
    plot(xaxis(1:(globalnbins(jxyz))), -squeeze(mean(mean(ave_pressure_surface(:,:,:,ixyz),1),3)), ...
         'x','LineWidth',5,'Color',[.5 .5 .5],'MarkerSize',20);
    hold on    
       
    %axis([-0.1 1.1 -0.5 0.1])
    %Plot continuum stress profile
    %plot(xaxis(globalnbins(jxyz)-overlap:end),continuum_stress(:,m),'s','Color',[.5 .5 .5],'MarkerSize',20,'LineWidth',5);
