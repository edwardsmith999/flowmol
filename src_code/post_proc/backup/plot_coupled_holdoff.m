...
for i = 1:Nvel_records
    ...
    plot(xaxis(1:(globalnbins(ixyz))),ave_vel_slice(:,1,m-1)/u_0,'x','LineWidth',5,'Color',[.5 .5 .5],'MarkerSize',20);
    ...
    plot(xaxis(((globalnbins(ixyz)-1))-overlap+1:end-1),continuum_velslice(2:ny+1,(m-0.5)*t_ave)/u_0,'s','Color',[.5 .5 .5],'MarkerSize',20,'LineWidth',5);
    %Plot Halo values
    plot(xaxis(end),continuum_velslice(end,(m-0.5)*t_ave)/u_0,'o','Color',[.5 .5 .5],'MarkerSize',20,'LineWidth',3,'HandleVisibility','off');
    plot(xaxis(((globalnbins(ixyz)-1))-overlap),continuum_velslice(1,(m-0.5)*t_ave+1)/u_0,'o','Color',[.5 .5 .5],'MarkerSize',20,'LineWidth',3);
