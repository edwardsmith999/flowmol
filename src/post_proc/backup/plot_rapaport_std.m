...
resultfile_dir = '/home/es205/codes/issued_codes/svn_lucian/MD_dCSE/src_code/results/rapaport_std/'
...
for i=1:(Nsteps-initialstep)/tplot
    ...
    [velocity_snapshot_t,velocity_flux_t,pressure_surface_t] = read_vflux(t,resultfile_dir,globalnbins,nd);
...
figure(fig2)
virial = macroscopic_properties(:,9);
virial_kinetic = macroscopic_properties(:,9)-macroscopic_properties(:,8);
virial_config = macroscopic_properties(:,8);
plot(virial_kinetic,'r','LineWidth',3)
legend('CV kinetic pressure','kinetic virial')

figure(fig1)
plot(virial_config,'--r','LineWidth',3)
...
Ns=1;
clear a; clear b; clear virial_std
for n=1:13
    blocks=size(1000:size(CV_virial,1)-Ns,2)/Ns+1
...
%     end
    m = 1; virial_std=0; virial_std2=0; config_virial_std=0; kinetic_virial_std=0;
    for i = 1000:Ns:size(virial,1)-Ns
        %virial_std(i)=std(macroscopic_properties(2+(i-1)*Ns:2+i*Ns,9));
        mean_virial = mean(virial(1000:i+Ns));
        virial_std          = virial_std         + (mean(virial(i:i+Ns-1).^2)         - mean(virial(1000:end))^2);
        virial_std2         = virial_std2       + (mean(virial(i:i+Ns-1).^2)         - mean_virial^2);
        config_virial_std  = config_virial_std  + (mean(virial_config(i:i+Ns))^2  - mean(virial_config)^2);
        kinetic_virial_std = kinetic_virial_std + (mean(virial_kinetic(i:i+Ns))^2 - mean(virial_kinetic)^2);
    end
%     a(n,1) = mean(CV_config_std);
%     a(n,2) = mean(CV_kinetic_std);
%     a(n,3) = mean(config_virial_std);
%     a(n,4) = mean(kinetic_virial_std);
%     b(n,1) = mean(CV_std);
    b(n,1) = virial_std /(blocks-1);
    b(n,2) = virial_std2/(blocks-1);
   % b(n,2) = config_virial_std/(blocks-1);
   % b(n,3) = kinetic_virial_std/(blocks-1);
    xaxis(n) = Ns;
    Ns = Ns*2;
end
%plot(sqrt(a))
...
xaxis = 1:13
figure
scatter(xaxis,sqrt(b(:,1)))
hold all
scatter(xaxis,sqrt(b(:,2)))
rapaport = [12 17 23 31 39 44 46 47 40 44 45 50 33]*0.001;
scatter(xaxis,rapaport)
legend('std virial','std Virial2', 'rapaport')
