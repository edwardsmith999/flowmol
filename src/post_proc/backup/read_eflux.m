...
function[energy_snapshot_t,energy_flux_t,energy_surface_t] = read_eflux(read_time,resultfile_dir,globalnbins)
...
%properties
Ncubeface = 6;
datasize = globalnbins(1)*globalnbins(2)*globalnbins(3)*Ncubeface;
bytes = 8;
...
energy_flux_t = reshape(eflux,globalnbins(1),globalnbins(2),globalnbins(3),Ncubeface);
fclose(fid);
...
energy_surface_t = reshape(esurface,globalnbins(1),globalnbins(2),globalnbins(3),Ncubeface);
fclose(fid);
...
energy_snapshot_t = reshape(esnap,globalnbins(1),globalnbins(2),globalnbins(3));
fclose(fid);