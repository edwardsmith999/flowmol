...
for i = 1:Nvel_records
    ...
    plot(xaxis(1:(globalnbins(ixyz))),mean(density_slice(:,:),2),'--');
    hold on
    plot(xaxis(1:(globalnbins(ixyz))),density_slice(:,m),'r-');
    ...
    %    t = 0;
    %end
    analy = couette_analytical_fn(t,Re/liquiddomain(ixyz),[2,0], ...
        liquiddomain(ixyz), ...
        analy_points,'top');
    ...
    %plot(xaxis(1:(globalnbins(ixyz))),ave_vel_slice(:,1,m)/u_0,'x','LineWidth',5,'Color',[.5 .5 .5],'MarkerSize',20);
    plot(xaxis(1:(globalnbins(ixyz))),-mean(ave_vel_slice(:,1,m-t_ave/2:m+t_ave/2),3),'x','LineWidth',5,'Color',[.0 .5 .0],'MarkerSize',20);
    
    %plot(xaxis(1:(globalnbins(ixyz))),continuum_velslice(2:ny+1,m)/u_0,'s','Color',[.5 .5 .5],'MarkerSize',20,'LineWidth',5);
    ...
    %plot(xaxis(((globalnbins(ixyz)-1))-overlap+1:end-1),continuum_velslice(2:ny+1,m)/u_0,'s','Color',[.5 .5 .5],'MarkerSize',20,'LineWidth',5);
    %Plot Halo values
   % plot(xaxis(end),continuum_velslice(end,m)/u_0,'o','Color',[.5 .5 .5],'MarkerSize',20,'LineWidth',3,'HandleVisibility','off');
    %plot(xaxis(((globalnbins(ixyz)-1))-overlap),continuum_velslice(1,m+1)/u_0,'o','Color',[.5 .5 .5],'MarkerSize',20,'LineWidth',3);
