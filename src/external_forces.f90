...
subroutine sine_force(F_ext, wavenum, magnitude)
	...
	real(kind(0.d0)), intent(in)	:: F_ext
	real(kind(0.d0)), dimension(3), intent(in)	:: wavenum, magnitude
	...
	do n=1,np
		...
		!Fapplied(:) = magnitude(:)*(1.d0-cos(wavenum(1)*pi*rmapped(1))) & 
		!						  *(1.d0-cos(wavenum(2)*pi*rmapped(2))) & 
		!						  *(1.d0-cos(wavenum(3)*pi*rmapped(3)))
		Fapplied(:) = F_ext*magnitude(:)*(-sin(wavenum(:)*pi*rmapped(:)))
        !print'(a,i7,9f10.5)', "sine_force", n, rmapped, Fapplied, a(:,n)
		a(:,n)= a(:,n) + Fapplied(:) 
	enddo

end subroutine sine_force
...
subroutine get_Fstresses_mol(CFD_stress,   & 
    ...
    do kbin = 2,nbins(3)+1
        do ixyz = 1,3
	        Pxy(ibin,jbin,kbin,ixyz,1) =  Pxy(ibin,jbin,kbin,ixyz,1) & 
                                  /dble(boxnp(ibin,jbin,kbin))
	        Pxy(ibin,jbin,kbin,ixyz,2) =  Pxy(ibin,jbin,kbin,ixyz,2) & 
                                  /dble(boxnp(ibin,jbin,kbin))
	        Pxy(ibin,jbin,kbin,ixyz,3) =  Pxy(ibin,jbin,kbin,ixyz,3) & 
                                  /dble(boxnp(ibin,jbin,kbin))
	        Pxy(ibin,jbin,kbin,ixyz,4) = -Pxy(ibin,jbin,kbin,ixyz,4) & 
                                  /dble(boxnp(ibin,jbin,kbin))
	        Pxy(ibin,jbin,kbin,ixyz,5) = -Pxy(ibin,jbin,kbin,ixyz,5) & 
                                  /dble(boxnp(ibin,jbin,kbin))
	        Pxy(ibin,jbin,kbin,ixyz,6) = -Pxy(ibin,jbin,kbin,ixyz,6) & 
                                  /dble(boxnp(ibin,jbin,kbin))
        enddo
    enddo
    enddo
...
subroutine get_F_correction_CV(u_CFD, &
	...
	integer,parameter									 :: iter_correct=20
	integer,dimension(3)								 :: bin
	...
	do k = lbl(5),lbl(6)
		...
		if (.not. correction_lock(i,j,k)) then
	        ...
	        if (any(abs(u_error(i,j,k,:)) .gt. tol)) then
...
subroutine check_CFD(u_CFD, u_CFD_mdt, CFD_stress, CFD_flux, lbl)
	...
	do k = lbl(5),lbl(6)
		...
		if (abs(conserved) .gt. tol) then
            if (sum(dvelocitydt) .gt. tol) then
    			print'(a,i8,4i4,e12.3,3f9.4,f18.12,2f12.7)','Error_in_CFD_momentum_flux', iter,irank,i,j,k, & 
...
subroutine check_CFD_vs_MD(u_CFD, lbl, outtype)
	...
	integer,intent(in)             		                :: lbl(6),outtype
	real(kind(0.d0)),intent(in), & 
        allocatable,dimension(:,:,:,:) 	                :: u_CFD
...
subroutine apply_CV_force()
	...
	real(kind(0.d0)),allocatable,dimension(:,:,:,:),save:: u_CFD_mdt
    ...
    elseif (iter-initialstep+1 .eq. CVforce_starttime) then
...
#if USE_COUPLER 
        !Check bin sizes are consistent with coupled case
        ...
        !Allocare array to save CFD value from previous timestep
    ...
    call check_CFD_vs_MD(u_CFD_mdt, lbl, 3)
	...
	elseif (CVforce_correct .eq. 1) then
		call get_F_correction_CV(u_CFD_mdt, lbl, Fcorrect_CV, CVforce_applied_dir)
	endif
