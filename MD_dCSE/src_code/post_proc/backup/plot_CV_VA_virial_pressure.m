...
resultfile_dir = '/home/es205/codes/issued_codes/svn_lucian/MD_dCSE/src_code/results/120215_std_study'
...
CV_virial = mean(pressure_config(2:end,:),2) + mean(pressure_kinetic(2:end,:),2);
figure(fig1)
plot(mean(pressure_config(2:end,:),2),'b','LineWidth',3)
hold on
figure(fig2)
plot(mean(pressure_kinetic(2:end,:),2),'b--','LineWidth',3)
hold on
%===================Read VA virial====================
read_pVA
...
legend('CV config pressure','VA config pressure','config virial')

figure(fig2)
plot(macroscopic_properties(2:end,8),'--r','LineWidth',3)
legend('CV kinetic pressure','VA kinetic pressure','kinetic virial')
...
for i = 10:size(CV_virial,1)-Ns
	a(i) = std(CV_virial(i:i+Ns),1);
end
plot(mean(a(1:end,:),2),'c')
for i = 10:size(macroscopic_properties(2:end,9),1) - Ns
    %virial_std(i)=std(macroscopic_properties(2+(i-1)*Ns:2+i*Ns,9));
    virial_std(i)=std(macroscopic_properties(i:i+Ns,9));
end
plot(virial_std)

legend('Direct Pressure VA','Shear Pressure VA','CV direct','Virial')
