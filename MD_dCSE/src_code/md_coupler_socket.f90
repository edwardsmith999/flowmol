...
subroutine apply_continuum_forces(iter)
	use coupler
	use computational_constants_MD, only : delta_t, nh, ncells, cellsidelength, halfdomain
	use linked_list
	use arrays_MD, only : r, v, a
	implicit none
	...
	real(kind(0.d0)), allocatable :: vel_cfd(:,:,:,:,:)
	logical overlap
	...
	if (overlap) then 
		...
		nib = nint((xmax - xmin)/ dx_cfd)
		njb = 1
		nkb = nint((zmax - zmin)/ dz_cfd)
		
		allocate(vel_cfd(3,nib,njb,nkb,2))
...
!	find the maximum number of molecules and allocate a list array	   
	...
	! one has to treat the particle that have left the domain separately
...
!-----------------------------------------------------------------------------
	...
	subroutine average_over_bin
		...
		do i = 1, np_overlap
			...
			alpha(1) = inv_dtCFD*(vel_cfd(1,ib,1,kb,1) - &
								  vel_cfd(1,ib,1,kb,2))

			u_cfd_t_plus_dt(1) = alpha(1) * (iter + 1)*delta_t + vel_cfd(1,ib,1,kb,1) 
...
subroutine apply_continuum_forces_ES(iter)
	...
	double precision, allocatable 		:: vel_cfd(:,:,:,:,:)
	logical								:: overlap
